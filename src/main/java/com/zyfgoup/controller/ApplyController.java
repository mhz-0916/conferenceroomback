...
public class ApplyController {
    ...
    public Result add(@RequestBody Record record) throws ParseException {


        Apply apply = record.getApply();
        ConferenceRecord conferenceRecord = record.getConferenceRecord();
//        applyService.save(apply);
        if (record.getApply().getRoomId() != 2){
            record.getApply().setAuditState(1);
            LocalDateTime localDateTime =LocalDateTime.now();
            record.getApply().setAuditTime(localDateTime);
        }
        applyService.save(apply);
        //要设置外键
    ...
    public Result searchtimeconflict(@PathVariable("roomId") Integer roomId,
                                     ...
                                     @PathVariable("endTime")  @DateTimeFormat(pattern="yyyy-MM-dd HH:mm:ss") LocalDateTime endTime) throws ParseException {

        // apply startTime endTime
        //       startTime1 endTime1
        // currentTime > startTime+10min room会议室.useStatus=未使用
        // --> apply

        LocalDateTime currentTime =LocalDateTime.now();
        Duration between = Duration.between(currentTime, startTime);
        long l = between.toMinutes();
        if(l>10 && conferenceRoomService.getById(roomId).getRoomUsestate()==1){
            Integer applyId = conferenceRecordService.searchApplyId(roomId);


            applyService.getById(applyId).setAuditState(2);
